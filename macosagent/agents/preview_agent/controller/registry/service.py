...
class Registry(Generic[Context]):
    ...
    async def execute_action(
        ...
        try:
            ...
            if "preview_context" in parameter_names:
                extra_args["preview_context"] = preview_context
            if "context" in parameter_names:
                extra_args["context"] = preview_context
            if is_pydantic:
            ...
            raise RuntimeError(f"Error executing action {action_name}: {e!s}") from e

    def _replace_sensitive_data(
        self, params: BaseModel, sensitive_data: dict[str, str]
    ) -> BaseModel:
    ...
    def create_action_model(
        self, include_actions: list[str] | None = None
    ) -> type[ActionModel]:
