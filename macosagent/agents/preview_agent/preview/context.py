...
from macosagent.agents.preview_agent.preview.preview import Preview
from macosagent.agents.preview_agent.preview.utils import (
    BoxDrawer,
...
class PreviewContext:
    def __init__(
        self,
        preview: 'Preview',
        state: PreviewContextState | None = None,
    ):
        self.context_id = str(uuid.uuid4())
        logger.debug(f'Initializing new browser context with id: {self.context_id}')
        self.preview = preview
        self.state = state or PreviewContextState()


    def get_state(self):
        windows, screenshots = self.preview.get_app_windows()
