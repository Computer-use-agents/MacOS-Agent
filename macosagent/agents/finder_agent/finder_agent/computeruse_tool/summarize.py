...
class Summarizer(Tool):
    ...
    def forward(self, task):
        ...
        # screenshot_folder_path=str(files("finder_agent").joinpath("screenshot"))
        screenshot_folder_path = str(Path('macosagent/agents/finder_agent/finder_agent').joinpath("screenshot"))
        
        if not Path(screenshot_folder_path).exists():
            Path(screenshot_folder_path).mkdir(parents=True, exist_ok=True)

        # screenshot_path=str(files("finder_agent.screenshot").joinpath("screenshot.png"))
        screenshot_path = str(Path('macosagent/agents/finder_agent/finder_agent/screenshot').joinpath("screenshot.pn"))
