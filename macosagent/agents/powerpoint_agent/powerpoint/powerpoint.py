...
def open_powerpoint(file_path=None):
	try:
		workspace = ApplicationServices.NSWorkspace.sharedWorkspace()
		# First launch powerpoint application
		workspace.launchApplication_("Microsoft PowerPoint")
		
		# Get powerpoint app from running applications
		running_apps = workspace.runningApplications()
		# print(running_apps)
		# exit()
		powerpoint_app = None
		for app in running_apps:
			if app.localizedName() == "Microsoft PowerPoint":
				powerpoint_app = app
				powerpoint_app.activateWithOptions_(0)
				break
				
		if not powerpoint_app:
			raise Exception("Failed to get Microsoft.PowerPoint application")
		# print(file_path)
		# exit()
		...
		print(f"Error opening PowerPoint: {str(e)}")
		raise
...
class PowerPoint:
	"""
	Open Mac powerpoint application and get the accessibility tree.
	"""

	def __init__(
		self,
		config: PowerPointConfig = PowerPointConfig(),
	):
		self.config = config
		#import os.path
		# if config.file_path:
		...
		# 	prs.save(config.file_path)
		
		# if config.file_path:
		# self.open_powerpoint(config.file_path)
		self.config.file_path = config.file_path
		self.powerpoint_app = None 

	def open_powerpoint(self, file_path=None):
		self.config.file_path = file_path
		#try:
		self.powerpoint_app = open_powerpoint(file_path)
		# except:
		# 	self.powerpoint_app = None
		self.app_ref = ApplicationServices.AXUIElementCreateApplication(self.powerpoint_app.processIdentifier())
			...
			if err == 0 and windows:
				...
				for i, window in enumerate(windows):
					...
					for window_info in window_list:
						if window_info.get(Quartz.kCGWindowOwnerPID) == self.powerpoint_app.processIdentifier():
...
if __name__ == "__main__":
	# pass
	powerpoint_app = PowerPoint(PowerPointConfig(file_path = './data/test.pptx'))
	# from context import PowerPointContext
	import os
	import time
	import uuid
	...
	powerpoint_context = PowerPointContext(powerpoint_app)
	# print(powerpoint_context.get_state())