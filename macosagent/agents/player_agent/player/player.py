...
from macosagent.agents.player_agent.player.utils import parse_axvalue_bounds
    ...
    try:
        ...
        for app in running_apps:
            if app.bundleIdentifier() == "com.apple.QuickTimePlayerX":
                player_app = app
                player_app.activateWithOptions_(0)
                break

        if not player_app:
            raise Exception("Failed to get QuickTime Player application")
        ...
        print(f"Error opening QuickTime Player: {e!s}")
        raise
...
class Player:
    """
    Open Mac QuickTime Player application and get the accessibility tree.
    """
    ...
    def __init__(
        ...
        self.config = config
        if config.file_path:
            self.open_player(config.file_path)

    def open_player(self, file_path=None):
        self.config.file_path = file_path
        self.player_app = open_player(file_path)
        self.app_ref = ApplicationServices.AXUIElementCreateApplication(self.player_app.processIdentifier())
            ...
            if err == 0 and windows:
                ...
                for i, window in enumerate(windows):
                    ...
                    for window_info in window_list:
                        if window_info.get(Quartz.kCGWindowOwnerPID) == self.player_app.processIdentifier():
