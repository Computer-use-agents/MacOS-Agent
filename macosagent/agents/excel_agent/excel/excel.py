...
from macosagent.agents.excel_agent.excel.utils import parse_axvalue_bounds
...
def open_excel(file_path=None):
	try:
		...
		workspace.launchApplication_("Microsoft Excel")
		...
		for app in running_apps:
			if app.localizedName() == "Microsoft Excel":
				excel_app = app
				excel_app.activateWithOptions_(0)
				break
				
		if not excel_app:
			raise Exception("Failed to get Microsoft.Excel application")
		# print(file_path)
		# exit()
		...
		if file_path:
			...
			# print(abs_path)
			os.system(f'open -a "Microsoft Excel" "{abs_path}"')
				
				# 使用AppleScript将Microsoft Excel
			os.system('''
			osascript -e 'tell app "Microsoft Excel" to activate'
			''')
			# exit()
...
class Excel:
	"""
	Open Mac Excel application and get the accessibility tree.
	"""
	...
	def __init__(
		...
		self.config = config
		if config.file_path:
			self.open_Excel(config.file_path)

	def open_excel(self, file_path=None):
		self.config.file_path = file_path
		self.excel_app = open_excel(file_path)
		self.app_ref = ApplicationServices.AXUIElementCreateApplication(self.excel_app.processIdentifier())
			...
			if err == 0 and windows:
				...
				for i, window in enumerate(windows):
					...
					for window_info in window_list:
						if window_info.get(Quartz.kCGWindowOwnerPID) == self.excel_app.processIdentifier():
...
if __name__ == "__main__":
	# pass
	excel_app = Excel(ExcelConfig(file_path = './data/test.pptx'))
	...
	class ExcelContext:
		def __init__(
			self,
			excel: 'Excel',
			state: Optional[ExcelContextState] = None,
		):
			self.context_id = str(uuid.uuid4())
			logger.debug(f'Initializing new browser context with id: {self.context_id}')
			self.excel = excel
			self.state = state or ExcelContextState()


		def get_state(self):
			windows, screenshots = self.excel.get_app_windows()
	...
	excel_context = ExcelContext(excel_app)
	# print(excel_context.get_state())